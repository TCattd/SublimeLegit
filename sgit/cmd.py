...
logger = logging.getLogger('SublimeLegit.cmd')


class SublimeLegitException(Exception):
    pass
...
class Cmd(object):
    ...
    def cmd(self, cmd, stdin=None, cwd=None, ignore_errors=False, encoding=None, fallback=None):
        ...
        try:
            ...
            raise SublimeLegitException("Could not execute command: %s" % e)
        except UnicodeDecodeError as e:
            if ignore_errors:
                return (0, '')
            sublime.error_message(self.get_decoding_error(encoding, fallback))
            raise SublimeLegitException("Could not execute command: %s" % command)
